...
-- SELECT * FROM employees WHERE salary > 63000 (not correct way)
...
SELECT * FROM employees WHERE salary > (SELECT max(salary) as max_salary FROM employees WHERE department_name = 'HR')

-- that query returns only one data that is called scaler sub query 
...
-- another use case of sub query 

-- we can use this sub query on SELECT FROM WHERE
...
SELECT * FROM employees
    WHERE
    salary > (SELECT max(salary) as max_salary FROM employees WHERE department_name = 'HR');
...
-- FORM -- sub query depends on the outer query (so main query and sub query ek hote hobe)
-- WHERE -- sub query depends what condition ur using or comparison
